...
public:
    ...
    void Bind_Broadcast(std::string IPv4, std::uint16_t port, FDR_ON_ERROR)
    {
        if (inet_pton(AF_INET, IPv4.c_str(), &this->address.sin_addr) <= 0)
        {
            onError(errno, "Invalid address. Address type not supported.");
    ...
    void Bind_Broadcast(std::uint16_t port, FDR_ON_ERROR)
    {
        this->Bind_Broadcast("0.0.0.0", port, onError);
    }
};